...
a few default mappings for integers, floating point numbers, and strings.
For the classes, XS++ will automatically create a mapping of type C<O_OBJECT>
which uses the de-facto standard way of storing a pointer to the C(++)
object in the IV slot of a referenced/blessed scalar. If you deal with any
...
In newer versions of the XS compiler, (C<ExtUtils::ParseXS> version 3 or better),
you can embed your XS typemaps into XS code. See a recent copy of L<perlxs>
for details.

Beware that right now, if you need to return an instance of one of your classes
from a method that is not a constructor, the default C<O_OBJECT> typemap will
fail because it expects a (C) variable C<CLASS> to contain the name of the
Perl class to bless into. Workaround demonstrated in F<examples/XSpp-Example>.
