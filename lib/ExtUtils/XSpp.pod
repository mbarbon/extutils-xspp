...
can be used to define a new type -> XS typemap mapping, with optional
...
C<%set> decorators synthesize a getter/setter named after the member
...
As an alternative, the class-level C<%accessors> decorator sets the
