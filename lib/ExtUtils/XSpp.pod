...
generated XS code.
  ...
  %typemap{<C++ type>}{parsed}{
      %cpp_type{<C++ type 2>};
      %call_function_code{% $2 = new Foo( $a ) %};
      %cleanup_code{% ... %};
      %precall_code{% ... %};

      # use only one of the following
      %output_code{% $1 = newSViv( $2 ) %};
...
C<$c> and C<$2> are replaced with the actual call code and the name of
...
C<$2> is replaced with the C++ return variable name.
...
special variables C<$1> and C<$2> are replaced with the names of the
...
calling the C++ method.  The special variables C<$1> and C<$2> are
...
to return a list.  The special variable C<$2> is replaced with the C++
