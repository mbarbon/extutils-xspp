...
sub output_code {
  ...
  return _replace( $_[0]->{OUTPUT_CODE}, '$1' => $pvar, '$2' => $cvar );
}

sub output_list {
  my( $this, $cvar ) = @_;
  return unless defined $_[0]->{OUTPUT_LIST};
  return _replace( $_[0]->{OUTPUT_LIST}, '$2' => $cvar );
}

sub cleanup_code {
  my( $this, $pvar, $cvar ) = @_;
  return unless defined $_[0]->{CLEANUP_CODE};
  return _replace( $_[0]->{CLEANUP_CODE}, '$1' => $pvar, '$2' => $cvar );
}
...
sub call_function_code {
  ...
  return _replace( $this->{CALL_FUNCTION_CODE}, '$1' => $func, '$$' => $var, '$2' => $var, '$c' => $func );
}
