...
A plugin can both run after the parsing is finished and modify the
expression tree before it is emitted and handle tags associated with a
...
During parsing no check is performed on parameter names/types/count.
The parser only gives an error if the tag is not handled by any
plugin.

Positional parameters are passed to tag handlers in the
C<any_named_arguments> parameter as an array reference, named handlers
are passed in the C<any_positional_arguments> parameter as an hash
reference.

If the parameter is an identifier its value is a plain string.  The
value of a special block parameter is an array reference with an
element for each line in the special block.
...
Register a post-processing plugin to be called after the parsing
finished.
...
This method is called for each loaded plugin.

TODO call this only once, and add another method that is called once for
each %loadplugin declaration.
