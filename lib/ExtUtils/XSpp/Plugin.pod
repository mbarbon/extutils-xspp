...
There are two ways a plugin can modify the code emitted by XS++: in
...
gives an error if the tag is not handled by any plugin.

Positional parameters are passed to tag handlers in the
C<any_named_arguments> parameter, as an array reference; named
handlers are passed in the C<any_positional_arguments> parameter, as
an hash reference.
